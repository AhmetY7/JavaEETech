...
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %> <%-- core tagimızı yükledik --%>
	...
	<div class="row">
		<div class="col-lg-6">
			...
			<table border="1">
				...
				<c:forEach items="${listUsers}" var="user"> <%-- listemizin üzerinde dönüyoruz. --%>
					<c:url var="updateURL" value="operation">
						<c:param name="page" value="updateUser"></c:param>			
						<c:param name="usersId" value="${user.users_id}"></c:param> <%-- Buradaki user foreachdeki var dan geliyor yani listusersin bir elemanı --%>
					...
					<c:url var="deleteUser" value="operation">
	 				...
	 				<tr>
		 				...
		 				<td>${user.email}</td>
		 				<td>
		 				<a href="${updateURL}">Update</a>| <%-- Yukarıdaki url tagindaki updateURL değişkeni  --%>
		 				<a href="${deleteUser}"
